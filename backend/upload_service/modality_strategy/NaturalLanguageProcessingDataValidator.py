...
class NaturalLanguageProcessingDataValidator(IValidatorStrategy):
    def __init__(self):
        self.dataset_rules = ['.txt']
        self.model_rules   = ['.pth']
        self.label_rules   = ['.txt', '.json']
        self.tokenizer_rules = ['.txt', '.json']
        ...
        match data_type:
            ...
            case "Label":
                if self.get_file_extension(file.name) in self.label_rules:
                    return {"message": "Valid", "status": status.HTTP_200_OK}
            case "Tokenizer":
                if self.get_file_extension(file.name) in self.tokenizer_rules:
                    return {"message": "Valid", "status": status.HTTP_200_OK}
